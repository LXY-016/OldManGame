...
*   **请求响应时间 (T-Request)**: 老人出现后头顶气泡存在的倒计时。
    *   **机制**: 若在倒计时结束前未派遣社工到达，则判定为**【失败】**（老人失望离开）。
...
**事件绑定规则 (Event Binding):**
1.  **伴生机制 (Bound on Spawn)**: 事件类型 (E-01~05) 在老人**生成的那一刻**即被确定，并与其生命周期强绑定。老人**携带**需求进入场景，而不是进入场景后等待系统分配。
2.  **生成节奏 (Pacing)**: 关卡配置表控制的是**“老人的生成频率”**，从而间接控制了“新事件的出现频率”。
3.  **互斥性 (Exclusive)**: 每个老人只携带 **1个** 固有需求，不可叠加。
4.  **固定性 (Immutable)**: 事件一旦生成，类型即锁定，在老人未离开前**绝对固定**。
5.  **权重判定**: 在生成老人时，根据权重随机决定该老人携带哪种需求。

我们将5类事件分为三种颜色等级，便于UI区分：

| 事件ID | 事件名称 | 描述 | 示例 | 对应属性需求 | UI建议颜色 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **E-01** | **老人**起居出行 | 搬重物、上下楼梯、扶起跌倒 | **强壮 (STR)** | 🟤 棕色/橙色 |
| **E-02** | **老人**社交互动 | 聊天解闷、读报纸、调解纠纷 | **健谈 (CHA)** | 🟢 绿色 |
| **E-03** | **老人**家具维修 | 换灯泡、修水管、调电视 | **聪明 (INT)** | 🔵 蓝色 |
| **E-04** | **老人**求医治病 | 陪诊、取药、心理疏导 | **健谈 (CHA)** | 🟢 绿色 |
| **E-05** | **老人**紧急健康 | 突发疾病急救、晕倒呼叫 | **强壮 (STR)** 或 **聪明 (INT)** | 🔴 红色 (高危) |
...
**建议数值设定：**
*   **基础完成时间 (Base Time)**: 详见 `gameconfig.md` [Section 4 - baseDuration]
*   **完美匹配减免**: 详见 `gameconfig.md` [Section 3 - reductionFactor]
*   **普通匹配减免**: 详见 `gameconfig.md` [Section 3 - 默认逻辑]

**运算示例：**
*   **场景**: 老人需要【修水管 (E-03)】。
    *   派去【聪明】的社工： 触发 **完美匹配**，时间大幅缩短。
    *   派去【强壮】的社工： 触发 **普通匹配**，无时间减免，占用社工较久。
